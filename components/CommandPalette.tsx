...
const CommandPalette: React.FC<CommandPaletteProps> = ({ 
  ...
  const actions = useMemo(() => [
    { 
      ...
      label: 'GO TO HOME', 
      icon: <Home size={20} />, 
      action: () => scrollToSection(NavSection.HERO) 
    },
    { 
      id: 'about', 
      label: 'GO TO ABOUT', 
      icon: <User size={20} />, 
      action: () => scrollToSection(NavSection.ABOUT) 
    },
    { 
      id: 'skills', 
      label: 'GO TO SKILLS', 
      icon: <Zap size={20} />, 
      action: () => scrollToSection(NavSection.SKILLS) 
    },
    { 
      id: 'projects', 
      label: 'GO TO PROJECTS', 
      icon: <Briefcase size={20} />, 
      action: () => scrollToSection(NavSection.PROJECTS) 
    },
    { 
      id: 'contact', 
      label: 'GO TO CONTACT', 
      icon: <Phone size={20} />, 
      action: () => scrollToSection(NavSection.CONTACT) 
    },
    { 
      id: 'theme', 
  ...
  // Navigation & Execution Listener (Active only when open)
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Number Hotkeys (1-8) mapped to VISIBLE items
                    ...
                    {filteredActions.length > 0 ? (
                      filteredActions.map((action, idx) => (
                          ...
                          >
                              {/* Keycap */}
                              <div className={`
                                w-8 h-8 flex items-center justify-center font-black text-lg border-2 
                              ...
                              {/* Label */}
                              <span className="font-bold uppercase tracking-wider flex-grow">
                                  {action.label}
                              </span>
